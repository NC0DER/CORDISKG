...
from CordisKG.utils import counter
...
def keybert(text, model, ngram_range = (1, 3), top_n = 10, measure = None, diversity = 0.5):
    ...
    if type(text) is str:
        ...
        return [[
            keyphrase for (keyphrase, _) in key_list] for key_list in 
            model.extract_keywords(
                text, 
                keyphrase_ngram_range = ngram_range,
                stop_words = 'english',
...
@counter
def yake(text, ngram_range = (1, 3), top_n = 10, dedupLim = 0.9, dedupFunc = 'seqm', windowsSize = 1):
